...
/* MSVC compatiblity: undefine max and min macros */
#if defined(max)
#undef max
#endif
...
void score_toplevel_move(board_t board, int move, char *resbuffer) {
	float res;
	...
	sprintf(resbuffer,"%d %f %ld %d %d %f %d ", move, res,
		state.moves_evaled, state.cachehits, (int)state.trans_table.size(), elapsed, state.maxdepth);
}


void subthread(board_t board, int tid, char* buffer) {
	char sbuf[128]={0};
	...
	close(file_descriptor[(bool)pid]);
	score_toplevel_move(board, tid<<1 | bool(pid), sbuf);
	if (!pid) {
		write(file_descriptor[1], sbuf, strlen(sbuf));
		close(file_descriptor[1]);
		exit(0);
	} else {
		int status;
		wait(&status);
		read(file_descriptor[0], buffer, sizeof sbuf);
		close(file_descriptor[0]);
		memcpy(buffer + strlen(buffer), sbuf, strlen(sbuf));
	}
}
...
int find_best_move(board_t board) {
	...
	char buffer[256]={0};
	close(file_descriptor[(bool)pid]);
	subthread(board, (bool)pid, buffer);
	if (!pid) {
		write(file_descriptor[1], buffer, strlen(buffer));
		close(file_descriptor[1]);
		exit(0);
	} else {
		int status;
	...
	char result[512]={0}, *readpos = result;
	read(file_descriptor[0], result, sizeof buffer);
	close(file_descriptor[0]);
	memcpy(result + strlen(result), buffer, strlen(buffer));

	float best = 0;
	int bestmove = -1;
	for (int offset, move=0; move<4; ++move, readpos += offset) {
		int tmove, cachehits, tablesize, maxdep;
		float res, elapsed;
		long evaled;
		sscanf(readpos, "%d%f%ld%d%d%f%d%n", &tmove, &res,
			&evaled, &cachehits, &tablesize, &elapsed, &maxdep, &offset);
		if (tmove != move) {
			fprintf(stderr, "Error in multithreading: move not match\n");
			exit(127);
		}
		printf("Move %d: result %f: eval'd %ld moves (%d cache hits, %d cache size) in %.3f seconds (maxdepth=%d)\n",
			move, res, evaled, cachehits, tablesize, elapsed, maxdep);
		if(res > best)
			best = res, bestmove = move;
	}
